...
void *client_comm(void *client_sock);
    ...
    while (true) {
        ...
        while (true) // wait for new client
        {
            ...
            if (l_read_poll[1].revents & POLLIN) { // new client?
                ...
                pthread_t handle;
                pthread_create(&handle, nullptr, client_comm, (void *) &l_sock_client);
...
void *client_comm(void *client_sock) {
    ...
    while (true) { // communication
        ...
        if (l_read_poll[0].revents & POLLIN) {
            ...
            // send data to client
            l_len = write(sockfd, l_buf, l_len);
            if (l_len < 0)
    ...
    if (close(sockfd)  < 0) {
        log_msg(LOG_ERROR, "Unable to close client socket!");
        exit(-1);
    }
    return nullptr;
}