...
int main(int t_narg, char **t_args) {
    ...
    while (true) {
        ...
        auto polret = poll(l_read_poll, 2, 2000);
        // select from fds
        if (polret < 0) break;
        ...
        if (polret == 0) {
            ...
            int sum = getpid();
            auto offset = sprintf(l_buf, "%d ", getpid());
            int num_of_digits = rand() % 30;
            for (int i = 0; i < num_of_digits; ++i) {
                int rnd = rand() % 10;
                sum += rnd;
                sprintf(l_buf + offset + (i * 2), "%d ", rnd);
            }
            l_i += 1;
            ...
            sprintf(l_buf + offset + num_of_digits * 2, "%d\n", sum);
            write(l_sock_server, l_buf, strlen(l_buf));
