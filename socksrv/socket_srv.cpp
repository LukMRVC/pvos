    ...
    while (true) {
        ...
        while (true) // wait for new client
        {
            ...
            if (l_read_poll[1].revents & POLLIN) { // new client?
                ...
                // fork server
                if (fork() == 0) {
                    // child can close listen socket
                    if (close(l_sock_listen) < 0) {
                        log_msg(LOG_ERROR, "Unable to close l_sock_listen in children");
                        exit(-1);
                    }
                    // get child away from this loop
                    break;
                } else {
                    // parent can close client socket
                    if (close(l_sock_client) < 0) {
                        log_msg(LOG_ERROR, "Unable to close l_sock_client in parent");
                        exit(-1);
                    }
                }
            }
